...
const generateCard = (tea) => {
    return (`
    <div class="tea-${tea.id} tea-div" style="width: 300px; margin: 20px;">
        <img src="${tea.image}" class="card-img-top" alt="Фото чайника" style="max-height:280px; ">
        <div class="tea-card-body">
            <h5 class="tea-title">${tea.name}</h5>
            <p class="tea-text">${tea.description}</p>
            <a href="#" class="btn btn-primary" style="background-color: yellow; color:black;">Подробнее</a>
    </div>
  </div>`)
}
const getTeas = async () => {
